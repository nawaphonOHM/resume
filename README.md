...
- **Parametric Circular Orbital Kinematics & Starter Coordinates**:
  The hero code badges orbit continuously around the central avatar along parametric circular paths. At component instantiation, the initial starter coordinates are computed directly from the current Unix epoch timestamp (`Date.now()`), after which continuous frame callbacks advance relative to high-resolution elapsed time. The left badge orbital frequency is scaled by the radius ratio ($r_{\text{right}} / r_{\text{left}}$):
...
| `src/app/resume/hero-section/hero-section.spec.ts`                                                                             | Dual-rate loop coordination: 1 Hz Bangkok clock ticker, Unix timestamp-initialized orbital starter coordinates, 60/120 Hz orbital kinematics, and reduced-motion clamping. |
