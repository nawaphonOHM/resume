...
All publishable résumé facts live in `src/app/data/resume/resume.data.ts` and conform to the contracts in `src/app/helper/resume-profile/resume-profile.interface.ts`. Update that data source rather than duplicating content in component templates.
